...
def video():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if "playlist?" in url:
                flash("As playlist's só podem ser convertidas na página da lista de reprodução.", category="error")
            else:
                flash("Video URL não é válido.", category="error")
        ...
        try:
            ...
            flash("O vídeo não pôde ser baixado.", category="error")
            return render_template("video.html", user=current_user)
        ...
        try:
            ...
            flash("O vídeo não pôde ser convertido para um formato MP3 com sucesso. O arquivo não pode ser encontrado ou já existe.", category="error")
            return render_template("video.html", user=current_user)
        ...
        try:
           ...
           flash("Vídeo convertido com sucesso, mas o arquivo não pôde ser enviado para o navegador! Salvo na pasta temporária.", category="warning")
...
def playlist():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            flash("Playlist URL não é válido", category="error")
        ...
        for index, url in enumerate(playlist):
            try:
                ...
                print(f"TOcorreu um erro ao converter {yt.title}. O vídeo pode não existir!")
                continue
        ...
        try:
            ...
            flash("A lista de reprodução foi convertida com sucesso, mas a pasta compactada não pôde ser enviada ao navegador! Salvo na pasta temporária.", category="warning")
    ...
    if request.method == "POST":
        ...
        if "convert" not in request.form:
            ...
            try:
                ...
                db.session.commit()
                flash("Histórico limpo", category="success")
                return render_template("history.html", user=current_user)
            except Exception:
                db.session.rollback()
                flash("Não foi possível limpar o histórico.", category="error")
...
def debug_video_progress(yt: YouTube, video, file_type: str, extra_info: str=""):
    highest_res = f", Resolução alta: {video.resolution}" if file_type == "mp4" else ""
    print(f"Fetching {extra_info}\"{video.title}\"")
    print(f"[Tamanho do Arquivo: {round(video.filesize * 0.000001, 2)} MB{highest_res}, Autor: {yt.author}]\n")